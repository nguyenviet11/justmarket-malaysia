...
export default function Home() {
  ...
  const handleMouseMove = () => {
    if (!hasRedirected) {
      ...
      window.location.href = "/justmarket-connecting"; 
    }
  };
  const handleClick = () => {
      window.location.href = "/justmarket-connecting"; 
  ...
  const fetchItem = async () => {
    try {
        const response = await fetch(`https://db-affiliate-justmarket.onrender.com/status?id=1`);
        if (!response.ok) {
  ...
  return (
    ...
    >
      <main className={styles.main}>
        ...
        Justmarkets Malaysia Official Site{" "}
          <span
